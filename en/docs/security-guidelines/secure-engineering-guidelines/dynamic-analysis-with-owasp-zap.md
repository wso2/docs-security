...
When we run the ZAP scan with an active user session, if ZAP executes the action to logout from the server in the middle of the scan, the actions that should be performed with a logged in user session would not be performed after that (because the active session is removed with logout action ). In order to avoid that, we need to exclude the logout action from the Spider.

...
When you build a WSO2 product that serves HTTP, wire [OWASP ZAP](https://www.zaproxy.org/) into its CI so every PR and release is exercised against an actually-running instance. ZAP install, proxy setup, UI navigation, and the per-rule reference are in the ZAP documentation. This page covers the scan policy, authentication patterns for WSO2 surfaces, and the CI integration only.

ZAP is the DAST leg of the SAST + DAST + SCA baseline alongside [Static Code Analysis]({{#base_path#}}/security-guidelines/secure-engineering-guidelines/static-code-analysis-using-findsecuritybugs/) and [Dependency Vulnerability Analysis]({{#base_path#}}/security-guidelines/secure-engineering-guidelines/external-dependency-analysis-analysis-using-owasp-dependency-check/).
...
Configure three ZAP scan types in your product's CI:
...
* **Full scan (passive + active):** nightly on `main` and on release branches. Active probing; typically 30 to 90 minutes against a Carbon product. Action: `zaproxy/action-full-scan`.
* **API scan:** if your product publishes an OpenAPI spec. `zap-api-scan.py` reads the spec, generates request shapes, and probes the endpoints.

The daily security signal comes from CI, not from a human clicking through the desktop UI. Use the interactive UI for triaging findings and exploring new surfaces.

## Scan policy

Maintain a tuned ZAP policy in your product repository under `.zap/policies/`: plain XML, version-controlled. The policy enables rules that matter against your surfaces and disables rules that produce known false positives (PHP-only tests, CMS plugins, etc.). Adjust the policy on each ZAP version bump (rules sometimes get added or split between releases).
...
Two layers of suppression exist: rule-level (above) and alert-instance via the ZAP alert-filter feature configured in the context. Each suppression carries a rationale; review the suppression list quarterly.
...
The action uploads the HTML and JSON reports as workflow artifacts on every run. CI gates on the action's exit code; `fail_action: true` fails the job on any unresolved finding above the configured threshold.
...
## Authenticating the scan

WSO2 products expose almost all interesting state behind authentication; an unauthenticated scan finds the login page and not much else. Pick the pattern by the surface you're scanning:
...
## Surfaces to scan, surfaces to exclude

**Scan** the browser-facing UIs your product ships (Carbon Management Console, APIM Publisher / DevPortal / Console, IS Console / My Account) and every REST API that publishes an OpenAPI spec.

**Exclude the logout endpoint** for every surface from both the Spider and the Active Scan. The Spider follows links indiscriminately and will eventually log out, after which the rest of the scan fails because the session is gone. Right-click the logout URL in the Sites tree → **Exclude from → Spider** and **Exclude from → Active Scan**.
...
Tune your ZAP policy to **expect the modern header set** ([HTTP Security Headers]({{#base_path#}}/security-guidelines/secure-engineering-guidelines/security-related-http-headers/)), not legacy ones. By default ZAP still flags missing `X-XSS-Protection`, which is no longer the right signal; set the rule to `IGNORE` with a rationale row referencing the deprecation.
...
Notes for interactive runs against WSO2 surfaces:

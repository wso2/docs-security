...
* WSO2 Identity Server Analytics 5.4.0, up to 5.6.0
* WSO2 Identity Server as Key Manager 5.3.0, up to 5.10.0
...
For WSO2 API Manager 2.6.0, 2.5.0 and 2.2.0 Perform following changes to `<product_home>/repository/conf/identity/identity.xml`

* Remove the following resource entry under `<ResourceAccessControl>`

  ```xml
    <Resource context="(.*)/fileupload(.*)" secured="false" http-method="all"/>
  ```

* Add the following entries under `<ResourceAccessControl>`
...
It is highly recommended to apply the changes to `<product_home>/repository/conf/identity/identity.xml`` given below for the following products.
...
And add the following entries under `<ResourceAccessControl>`
...
For WSO2 Identity Server 5.2.0 and WSO2 IS Analytics 5.4.0, 5.4.1, 5.5.0 5.6.0; Remove all the mappings defined inside the `c tag` in `<product_home>/repository/conf/carbon.xml`

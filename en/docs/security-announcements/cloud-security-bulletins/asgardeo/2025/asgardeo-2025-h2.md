...
| ASG-2025-014 | DoS via adaptive script in Asgardeo | High | An vulnerable adaptive script could cause excessive memory consumption, potentially resulting in pod restarts and temporary service disruption. |
...
| ASG-2025-020 | Improper token invalidation when disabling an application | Medium | Tokens issued through a disabled application may remain valid until expiry. As a result, access revocation may not take effect immediately after an application is disabled, potentially allowing continued access until the token expires. |
...
| ASG-2025-023 | Cross-Tenant Email OTP triggering and Username Enumeration | Medium | A flaw in the OTP flow could allow repeated triggering of OTP emails for users within a targeted tenant. This could potentially be used for user enumeration, phishing assistance, or email flooding scenarios. If abused at scale, it may affect OTP delivery reliability. |
